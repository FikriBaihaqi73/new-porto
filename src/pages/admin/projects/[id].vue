  ...
  <div class="min-h-screen bg-gradient-to-br from-[#0f0f1c] to-[#1a1a2e] text-white flex">
    ...
    <div class="flex-1 flex flex-col">
      ...
      <main class="flex-1 p-6">
        <h1 class="text-4xl font-bold mb-8 text-blue-400">{{ projectId ? 'Edit Project' : 'Create New Project' }}</h1>
        ...
        <form @submit.prevent="submitForm" class="bg-[#1f2937]/80 p-8 rounded-lg shadow-xl border border-blue-700/50 backdrop-filter backdrop-blur-sm max-w-4xl mx-auto">
          <div v-if="loading" class="text-center text-teal-300">Loading project data...</div>
...
const fetchProject = async () => {
  ...
  error.value = null;
  try {
    const response = await $fetch<{ status: number; data: Project; message?: string }>(`/api/projects/${projectId}`);
    if (response.status === 200) {
      form.value = { ...response.data, techStack: response.data.techStack as string[] };
    } else {
      error.value = response.message || 'Failed to fetch project';
    }
  } catch (err: any) {
    error.value = err.data?.statusMessage || err.message || 'Error fetching project';
    console.error(err);
  } finally {
    loading.value = false;
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    alert(err.data?.statusMessage || err.message || 'Error updating project');
    console.error(err);
  } finally {
